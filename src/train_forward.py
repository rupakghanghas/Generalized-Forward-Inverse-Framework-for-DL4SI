...
def train_one_epoch(model, vgg_model, masked_criterion, optimizer, lr_scheduler, 
    ...
    amp_vgg_loss = torch.tensor([0.], device=device)
    ...
    for mask, amp, vel in metric_logger.log_every(dataloader, print_freq, header):
        ...
        loss = args.lambda_amp * amp_loss  + args.lambda_vgg_amp * amp_vgg_loss
...
def evaluate(model, criterion, dataloader, device, ctx, transform_data=None, transform_label=None):
    ...
    with torch.no_grad():
        ...
        eval_metrics = ["amp_sum_abs_error", "amp_sum_squared_error"]
        eval_dict = {}
...
def main(args):
    ...
    if args.model not in forward_network.model_dict:
        print('Unsupported model.')
        sys.exit()
    
    def set_forward_params(args, forward_model_params):
        forward_model_params.setdefault('IUnetForwardModel', {})
        forward_model_params['IUnetForwardModel']['cfg_path'] = args.cfg_path
        forward_model_params['IUnetForwardModel']['latent_dim'] = args.latent_dim
        
        forward_model_params.setdefault('UNetForwardModel', {})
        forward_model_params['UNetForwardModel']['cfg_path'] = args.cfg_path
        forward_model_params['UNetForwardModel']['latent_dim'] = args.latent_dim
        forward_model_params['UNetForwardModel']['unet_depth'] = args.unet_depth
        forward_model_params['UNetForwardModel']['unet_repeat_blocks'] = args.unet_repeat_blocks
        forward_model_params['UNetForwardModel']['skip'] = args.skip
        return forward_model_params
    
    # creating forward model
    forward_model_params = forward_network.forward_params
    forward_model_params = set_forward_params(args, forward_model_params)

    model = forward_network.model_dict[args.model](**forward_model_params[args.model]).to(device)
...
def parse_args():
    ...
    # Model related
    parser.add_argument('-m', '--model', type=str, help='forward model name')
    ...
    parser.add_argument('--skip', default=1, type=int, choices=[0, 1], help="Unet skip connections 0:False, and 1:True")
    ...
    parser.add_argument('--unet_depth', type=int, default=2, help='Depth of UNet Model')
    parser.add_argument('--unet_repeat_blocks', type=int, default=2, help='Number of Repeating Blocks for each depth of UNet Model')
